  ...
  return (
    <main className="login-shell">
      <form className="login-card" onSubmit={submit}>
        ...
        <p className="eyebrow">XGuard Admin</p>
        <h1>管理画面へログイン</h1>
        <p>{notice}</p>
...
function Dashboard({ accessToken, member }: { accessToken: string; member: AdminMember }) {
  ...
  return (
    <>
      ...
      <p className="page-notice">{notice}・権限: {member.role}</p>
      <section className="metric-grid" aria-label="Database snapshot">
        {(snapshot?.tables ?? []).map((table) => (
          <article className="metric-card" key={table.name}>
...
function TeamPage({
  ...
  return (
    <>
      ...
      <p className="page-notice">{notice}</p>
      ...
      <div className="member-list">
        {members.map((entry) => (
          <article className="member-row" key={entry.id}>
            <div>
